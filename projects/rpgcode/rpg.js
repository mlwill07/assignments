...
function getEnemies() {
    var enemyarray = ['Big Bad', 'Baby Alligator', 'slug'];
    ...
    function fightInterupter(isTurn) {
        ...
        if (isTurn === undefined) {
            var key = readline.keyIn("Are you ready?\n yes [y] ?");
        } else if (!isTurn) {
            console.log("Prepare to be attacked!");
            var key = readline.keyIn("Are you ready?\n yes [y]?");
        } else if (isTurn) {
    ...
    function battle() {
        ...
        if (foolishOne.health < 1) {
            console.log("\nOHHH Maximum damage. poor little human....\n")
    ...
    function getStuff() {
        ...
        var items = ['useless dagger', 'broken compass', 'hole-y shoe', 'dirty napkin', 'glow stick of destiny', 'lint', 'tuna sandwich', 'moldy cheese'];
        var bonusitem = items[randomN(0, 8)];
        foolishOne.uselessInventory.push(bonusitem);
        foolishOne.level++;
        var getpotion = randomN(0, 1);
        if (getpotion === 0) {
            console.log('\nSwag is awesome!\n\nYou got ' + bonuspoints + ' extra health\n\nand\na ' + bonusitem.toUpperCase() + ' was added to your useless inventory!');
        } else if (getpotion === 1) {
            foolishOne.healingPotions += 1;
            console.log('\nSwag is awesome!\n\nYou got ' + bonuspoints + ' extra health\n\nand\na ' + bonusitem.toUpperCase() + ' was added to your useless inventory!');
            console.log('\nRemember at the main prompt you can see your inventory by pressing [i]!');
        }
        console.log('Do you want to see your stats? yes [y] | no [n] ?');
